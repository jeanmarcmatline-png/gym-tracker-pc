...
function renderCal(){
  ...
  for(var i=startOff-1;i>=0;i--){
    html+='<div class="cal-day other-month">'+(dimPrev-i)+'</div>';
  }
  for(var d=1;d<=dim;d++){
...
async function loadExercises(){
  var r=await fetch('/api/exercises');
...
async function addEx(){
  ...
  await fetch('/api/exercises',{
    ...
    body:JSON.stringify({
      name:n,
      muscle_group:document.getElementById('ex-grp').value,
...
function renderLibrary(){
  ...
  document.getElementById('lib-filters').innerHTML=groups.map(function(g){
    return '<button class="tf'+(g===libFilter?' on':'')+'" onclick="libFilter=\''+g+'\';renderLibrary()">'+g+'</button>';
  }).join('');
  ...
  if(!filtered.length){
    document.getElementById('lib-list').innerHTML='<p style="color:var(--muted);font-size:13px;padding:8px 0">Aucun exercice.</p>';
    return;
  }
  document.getElementById('lib-list').innerHTML=filtered.map(function(e){
    ...
    return '<div class="ex-item">'+
      '<div>'+
        '<div style="font-size:13px;font-weight:500;margin-bottom:2px">'+e.name+(e.is_compound?'<span class="badge b-p" style="margin-left:5px">&#9733;</span>':'')+'</div>'+
        '<div style="font-size:11px;color:var(--muted)">'+e.muscle_group+' &middot; <span class="badge '+bt+'">'+tl[e.type]+'</span></div>'+
      '</div>'+
      '<button class="btn sm danger" onclick="delEx('+e.id+')">Suppr.</button>'+
    '</div>';
  }).join('');
}
...
async function loadCycles(){
  var r=await fetch('/api/cycles');
  cycles=await r.json();
  var sel=document.getElementById('sel-cycle');
  var prev=sel.value;
  sel.innerHTML='<option value="">— Aucun —</option>'+cycles.map(function(c){
...
function buildDays(){
  ...
  if(daysStruct.length!==n){
    daysStruct=[];
    for(var i=0;i<n;i++) daysStruct.push({name:'Jour '+(i+1),exercise_ids:[],is_rest:false});
  }
  renderDaysBuilder();
}
...
function renderDaysBuilder(){
  document.getElementById('days-bld').innerHTML=daysStruct.map(function(d,i){
    var chips=d.exercise_ids.map(function(eid){
      var ex=exercises.find(function(e){return e.id===eid;});
      return ex?'<div class="chip"><span>'+ex.name+'</span><button class="chip-x" onclick="rmExDay('+i+','+eid+')">&#215;</button></div>':'';
    }).join('');
    return '<div class="day-bld">'+
      '<div class="day-bld-head">'+
        '<span class="day-num">J'+(i+1)+'</span>'+
        '<input type="text" value="'+d.name+'" onchange="daysStruct['+i+'].name=this.value" style="flex:1;min-width:0">'+
        '<label class="tog">'+
          '<input type="checkbox"'+(d.is_rest?' checked':'')+' onchange="togRest('+i+',this.checked)">'+
          '<span class="tog-track"></span>'+
          '<span class="tog-thumb"></span>'+
          '<span style="margin-left:4px">Repos</span>'+
        '</label>'+
        (!d.is_rest?'<button class="btn sm" onclick="openPicker('+i+')">+ Exercices</button>':'')+
      '</div>'+
      (d.is_rest
        ?'<span style="font-size:12px;color:var(--muted);font-style:italic">Repos / récupération active</span>'
        :'<div class="chips">'+chips+(chips?'':'<span style="font-size:12px;color:var(--muted)">Aucun exercice</span>')+'</div>'
      )+
    '</div>';
  }).join('');
}
function togRest(i,v){
  daysStruct[i].is_rest=v;
  if(v) daysStruct[i].exercise_ids=[];
  renderDaysBuilder();
}
function rmExDay(i,eid){
  daysStruct[i].exercise_ids=daysStruct[i].exercise_ids.filter(function(e){return e!==eid;});
  renderDaysBuilder();
}
async function saveCycle(){
  ...
  var r=await fetch('/api/cycles',{
    method:'POST',
    headers:{'Content-Type':'application/json'},
    body:JSON.stringify({name:name,days:daysStruct})
  });
  var data=await r.json();
  if(!data.ok){toast('Erreur sauvegarde','warn');return;}
  await loadCycles();
  document.getElementById('sel-cycle').value=data.id;
  await setActiveCycle(data.id);
  renderCyclesList();
...
function renderCyclesList(){
  ...
  if(!cycles.length){
    el.innerHTML='<div class="empty"><div class="empty-ico">&#8862;</div><p>Aucun programme.</p></div>';
    return;
  }
  el.innerHTML=cycles.map(function(c){
    return '<div class="cy-row">'+
      '<div>'+
        ...
        '<div style="font-size:11px;color:var(--muted);font-family:\'DM Mono\',monospace">'+c.created_at+'</div>'+
      '</div>'+
      '<div style="display:flex;gap:6px;flex-wrap:wrap">'+
        '<button class="btn sm teal" onclick="showCompo('+c.id+')">Soumettre à Claude</button>'+
        '<button class="btn sm" onclick="editCycle('+c.id+')">Éditer</button>'+
        '<button class="btn sm danger" onclick="delCycle('+c.id+')">Suppr.</button>'+
      '</div>'+
    '</div>';
  }).join('');
}
async function editCycle(id){
  var r=await fetch('/api/cycles/'+id);
  var c=await r.json();
  document.getElementById('cy-name').value=c.name;
  document.getElementById('cy-days').value=c.days.length;
  daysStruct=c.days;
  renderDaysBuilder();
  goPage('cycles',document.querySelectorAll('.nav-item')[1]);
}
...
async function setActiveCycle(id){
  ...
  var r=await fetch('/api/cycles/'+id);
  activeCycle=await r.json();
  activeCycle.days=JSON.parse(activeCycle.days_json||'[]');
  renderSession();
}
async function showCompo(cid){
  var r=await fetch('/api/cycles/'+cid+'/composition');
  var d=await r.json();
  document.getElementById('co-text').value=d.composition||'Erreur';
  document.getElementById('co-overlay').classList.add('on');
}
async function copyCompo(){
  await navigator.clipboard.writeText(document.getElementById('co-text').value);
  toast('Copié — colle dans Claude');
}

// ── Picker ────────────────────────────────────────────────────────────────────
function openPicker(i){
  pickerDay=i;
  pickerSel=daysStruct[i].exercise_ids.slice();
...
function closePicker(){
  document.getElementById('pk-overlay').classList.remove('on');
}
function renderPicker(q){
  ...
  groups.forEach(function(g){
    ...
    fil.filter(function(e){return e.muscle_group===g;}).forEach(function(e){
      var s=pickerSel.indexOf(e.id)>=0;
      var bt=e.type==='strength'?'b-s':e.type==='hiit'?'b-h':'b-c';
      h+='<div class="pk-item'+(s?' sel':'')+'" onclick="togPick('+e.id+')">'+
        '<input type="checkbox"'+(s?' checked':'')+' onclick="event.stopPropagation();togPick('+e.id+')">'+
        '<span style="flex:1;font-size:13px">'+e.name+(e.is_compound?'<span class="badge b-p" style="margin-left:4px">&#9733;</span>':'')+'</span>'+
        '<span class="badge '+bt+'">'+tl[e.type]+'</span>'+
      '</div>';
    });
  });
...
function filterPicker(){
  renderPicker(document.getElementById('pk-search').value);
}
function togPick(id){
  var idx=pickerSel.indexOf(id);
  if(idx>=0) pickerSel.splice(idx,1);
  else pickerSel.push(id);
  renderPicker(document.getElementById('pk-search').value);
}
function confirmPicker(){
  daysStruct[pickerDay].exercise_ids=pickerSel.slice();
  closePicker();
  renderDaysBuilder();
}
...
function renderSession(){
  var ne=document.getElementById('sess-empty');
  var sm=document.getElementById('sess-main');
  if(!activeCycle){
...
async function loadSessionForm(){
  ...
  var last=exact;
  if(!exact){
    var rL=await fetch('/api/sessions/last?cycle_id='+activeCycleId+'&day_index='+curDay);
    last=await rL.json();
  }
  curFeeling=last?last.feeling:2;
  var data=last?last.data:{};
  var form=document.getElementById('sess-form');
  if(!day.exercise_ids||!day.exercise_ids.length){
  ...
  ['–','◎','+'].forEach(function(lbl,i){
    var v=i+1;
    html+='<button class="fb'+(curFeeling===v?' on':'')+'" onclick="setFeeling('+v+')">'+lbl+'</button>';
  });
  if(exact) html+='<span style="font-size:11px;color:var(--teal);margin-left:auto;font-family:\'DM Mono\',monospace">Session du '+curDate+' chargée</span>';
  ...
  day.exercise_ids.forEach(function(eid){
    var ex=exercises.find(function(e){return e.id===eid;});
    if(!ex) return;
    var isZ2=ex.type==='cardio_z2';
    var isHiit=ex.type==='hiit';
    var prev=data[eid]||{};
    var bt=isZ2?'b-c':isHiit?'b-h':'b-s';
    html+='<div class="ex-sec">'+
      '<div class="ex-sec-head">'+
        '<span class="ex-sec-name">'+ex.name+'</span>'+
        (ex.is_compound?'<span class="badge b-p">&#9733; Poly</span>':'')+
        '<span class="badge '+bt+'">'+ex.muscle_group+'</span>'+
      '</div>';
    ...
    if(isZ2||isHiit){
      ...
      html+='<div class="cmode-row">'+
        '<button class="cmode'+(mode==='z2'?' on':'')+'" id="cm-z2-'+eid+'" onclick="swCM('+eid+',\'z2\')">Zone 2</button>'+
        '<button class="cmode'+(mode==='hiit'?' on':'')+'" id="cm-hiit-'+eid+'" onclick="swCM('+eid+',\'hiit\')">HIIT</button>'+
      '</div>';
      if(mode==='z2'){
        var app2=(d2.appareil||'velo');
        html+='<div id="cm-body-'+eid+'">'+
          '<div class="cf-row"><label>Appareil</label>'+
            '<select id="c'+eid+'_app" style="width:160px">'+
              ...
              '<option value="rameur"'+(app2==='rameur'?' selected':'')+'>Rameur</option>'+
            '</select></div>'+
          '<div class="cf-row"><label>Durée totale</label><input type="number" id="c'+eid+'_dur" value="'+(d2.duree||'')+'" placeholder="40"><span class="unit">min</span></div>'+
          '<div class="cf-row"><label>FC moyenne</label><input type="number" id="c'+eid+'_fcm" value="'+(d2.fc_moy||'')+'" placeholder="110"><span class="unit">bpm</span></div>'+
          '<div class="cf-row"><label>FC max</label><input type="number" id="c'+eid+'_fcx" value="'+(d2.fc_max||'')+'" placeholder="118"><span class="unit">bpm</span></div>'+
          '<div style="font-size:11px;color:var(--muted);margin-top:5px">Zone 2 cible : 101–118 bpm</div>'+
        '</div>';
      } else {
        var ivs=d2.intervals||[{duree:3,bpm:90,bpm_reel:0},{duree:2,bpm:140,bpm_reel:0}];
        var rounds=d2.rounds||4;
        html+='<div id="cm-body-'+eid+'">'+
          '<div class="cf-row"><label>Nombre de tours</label><input type="number" id="c'+eid+'_rounds" value="'+rounds+'" min="1" max="20" style="width:70px"></div>'+
          '<table class="set-tbl" style="margin-top:8px"><thead><tr>'+
            '<th style="text-align:left">Interv.</th><th>Durée min</th><th>BPM cible</th><th>BPM réel</th>'+
          '</tr></thead><tbody>';
        ivs.forEach(function(iv,i){
          html+='<tr>'+
            '<td class="sn">#'+(i+1)+'</td>'+
            '<td><input type="number" id="c'+eid+'_iv'+i+'_dur" value="'+(iv.duree||'')+'" min="0" step="0.5"></td>'+
            '<td><input type="number" id="c'+eid+'_iv'+i+'_bpm" value="'+(iv.bpm||'')+'"></td>'+
            '<td><input type="number" id="c'+eid+'_iv'+i+'_real" value="'+(iv.bpm_reel||'')+'"></td>'+
          '</tr>';
        });
        html+='</tbody></table></div>';
      }
    } else {
      html+='<table class="set-tbl"><thead><tr>'+
        '<th style="text-align:left">Série</th><th>Poids kg</th><th>Reps</th>'+
      '</tr></thead><tbody>';
      for(var s=1;s<=5;s++){
        ...
        html+='<tr class="'+(heavy?'s1bg':'')+'">'+
          ...
          '<td class="'+(heavy?'s1bg':'')+'"><input type="number" id="ex'+eid+'_s'+s+'_w" value="'+(ps.weight||'')+'" min="0" step="0.5" placeholder="—"></td>'+
          '<td><input type="number" id="ex'+eid+'_s'+s+'_r" value="'+(ps.reps||'')+'" min="0" placeholder="'+(heavy?'5-6':'10')+'"></td>'+
        '</tr>';
      }
      html+='</tbody></table>';
    }
  ...
  html+='<div class="card" style="margin-top:10px">'+
    ...
    '<input type="text" id="sess-note" value="'+(last&&last.note?last.note.replace(/"/g,'&quot;'):'')+'" placeholder="Douleur, variante, fatigue…">'+
  '</div>';
  html+='<div class="btn-row">'+
    '<button class="btn pri" onclick="saveSession()">&#10003; Enregistrer la séance</button>'+
  '</div>';
...
function swCM(eid,mode){
  var z2=document.getElementById('cm-z2-'+eid);
  var hiit=document.getElementById('cm-hiit-'+eid);
  if(z2) z2.classList.toggle('on',mode==='z2');
  if(hiit) hiit.classList.toggle('on',mode==='hiit');
}
...
async function saveSession(){
  ...
  day.exercise_ids.forEach(function(eid){
    ...
    if(isZ2||isHiit){
      ...
      if(mode==='z2'){
        sessionData[eid]={mode:'z2',
          appareil:(document.getElementById('c'+eid+'_app')||{value:'velo'}).value,
          duree:(document.getElementById('c'+eid+'_dur')||{value:0}).value,
          fc_moy:(document.getElementById('c'+eid+'_fcm')||{value:0}).value,
          fc_max:(document.getElementById('c'+eid+'_fcx')||{value:0}).value};
      } else {
        var rounds=parseInt((document.getElementById('c'+eid+'_rounds')||{value:4}).value)||4;
        var ivs=[];
        for(var i=0;i<2;i++){
          ivs.push({
            duree:parseFloat((document.getElementById('c'+eid+'_iv'+i+'_dur')||{value:0}).value)||0,
            bpm:parseInt((document.getElementById('c'+eid+'_iv'+i+'_bpm')||{value:0}).value)||0,
            bpm_reel:parseInt((document.getElementById('c'+eid+'_iv'+i+'_real')||{value:0}).value)||0
          });
        }
  ...
  if(m){
    var d=new Date(curDate+'T12:00:00');
    m.textContent='Enregistré — '+d.toLocaleDateString('fr-BE',{weekday:'long',day:'numeric',month:'long',year:'numeric'});
...
async function loadProgress(){
  ...
  var rh=await fetch('/api/sessions/history?cycle_id='+activeCycleId);
  var hist=await rh.json();
  var total=hist.length;
  var avgF=total?(hist.reduce(function(a,s){return a+(s.feeling||2);},0)/total).toFixed(1):'—';
  ...
  var rp=await fetch('/api/sessions/progression?cycle_id='+activeCycleId);
  var prog=await rp.json();
  var sel=document.getElementById('prog-ex');
  sel.innerHTML='<option value="">— Choisir un exercice —</option>'+
    Object.keys(prog).map(function(k){
      return '<option value="'+k+'">'+prog[k].name+'</option>';
    }).join('');
  // Réafficher le graphique si un exo était déjà sélectionné
  if(sel.value) drawChart();
}
async function drawChart(){
  var key=document.getElementById('prog-ex').value;
  if(!key) return;
  var r=await fetch('/api/sessions/progression?cycle_id='+activeCycleId);
  var prog=await r.json();
  var ex=prog[key];
  if(!ex||!ex.points.length){toast('Pas encore de données','warn');return;}
  ...
  progChart=new Chart(document.getElementById('prog-chart'),{
    ...
    data:{
      ...
      datasets:[
        ...
        {
          label:'Tonnage total (kg)',
          data:dataTonnage,
          borderColor:'#4ecba4',
          backgroundColor:'rgba(78,203,164,0.08)',
          tension:.35,fill:false,
          borderDash:[4,3],
          pointBackgroundColor:'#4ecba4',
    ...
    options:{
      ...
      plugins:{
        legend:{
          display:true,
          labels:{color:'#8888a0',font:{family:'DM Mono',size:11},boxWidth:12,padding:16}
        },
        tooltip:{
          callbacks:{
            label:function(ctx){
      ...
      scales:{
        x:{
          ticks:{color:'#8888a0',font:{family:'DM Mono',size:11}},
          grid:{color:'rgba(255,255,255,0.04)'}
        },
        yForce:{
          type:'linear',position:'left',
          ticks:{color:'#8b7cf8',font:{family:'DM Mono',size:11},callback:function(v){return v+'kg';}},
          grid:{color:'rgba(139,124,248,0.08)'},
          title:{display:true,text:'Force S1',color:'#8b7cf8',font:{size:11}}
        },
        yTonnage:{
          type:'linear',position:'right',
          ticks:{color:'#4ecba4',font:{family:'DM Mono',size:11},callback:function(v){return v+'kg';}},
          grid:{drawOnChartArea:false},
          title:{display:true,text:'Tonnage',color:'#4ecba4',font:{size:11}}
        }
      }
    }
  });
...
async function genReport(){
  if(!activeCycleId){
    document.getElementById('rep-area').value='Sélectionne un programme actif.';
    return;
  }
...
async function copyReport(){
  await navigator.clipboard.writeText(document.getElementById('rep-area').value);
  toast('Rapport copié');
}
...
async function exportMobile(){
  ...
  try{
    var r=await fetch('/api/export-mobile');
    if(!r.ok) throw new Error('Erreur serveur');
    var blob=await r.blob();
    var cd=r.headers.get('Content-Disposition')||'';
    var match=cd.match(/filename=([^\s;]+)/);
    var filename=match?match[1]:'config_mobile.json';
    var url=URL.createObjectURL(blob);
    var a=document.createElement('a');
    a.href=url; a.download=filename; a.click();
    URL.revokeObjectURL(url);
    if(msg){
      msg.textContent='Fichier généré : '+filename+' — Transfère-le dans OneDrive';
      setTimeout(function(){msg.textContent='';},5000);
    }
    toast('Config mobile générée');
  }catch(e){
    if(msg) msg.textContent='Erreur : '+e.message;
    toast('Erreur export','warn');
  }
}
...
async function importSession(evt){
  ...
  try{
    var r=await fetch('/api/import-session',{method:'POST',body:form});
    var data=await r.json();
    if(data.ok){
      ...
      // Recharger les données si le cycle actif est concerné
    ...
    if(msg){msg.textContent='Erreur : '+e.message;msg.style.color='var(--red)';}
    toast('Erreur import','warn');
  }
  evt.target.value='';
}
